...
[[nodiscard]] constexpr uint16_t LowPart32(uint32_t value) noexcept
{
    return static_cast<uint16_t>(value & 0xFFFF);
}

[[nodiscard]] constexpr uint32_t HighPart64(uint64_t value) noexcept
{
    return static_cast<uint32_t>(value >> 32);
}

[[nodiscard]] constexpr uint32_t LowPart64(uint64_t value) noexcept
{
...
// Returns index in [0, 31], 0 = least significant bit, 31 = most significant.
[[nodiscard]] uint32_t BitScanReverse32Portable(uint32_t mask) noexcept;
// Returns index in [0, 63], 0 = least significant bit, 63 = most significant.
[[nodiscard]] uint32_t BitScanReverse64Portable(uint64_t mask) noexcept;
...
[[nodiscard]] constexpr int CountBitsPortable(uint32_t x) noexcept
{
...
[[nodiscard]] constexpr int CountBitsPortable(uint64_t x) noexcept
{
    // https://en.wikipedia.org/wiki/Hamming_weight
...
[[nodiscard]] constexpr uint64_t ReverseBits64(uint64_t n) noexcept
{
    const uint64_t lo = ReverseBits32(static_cast<uint32_t>(n));
    const uint64_t hi = ReverseBits32(static_cast<uint32_t>(n >> 32));
    return (lo << 32) | hi;
}
...
// The input must be less than the maximum power of 2 representable by the type, otherwise the result is undefined.
// If x is already a power of 2, the result will be the next power of 2, RoundUpToNextPow2(16) will return 32.
// If x is 0, the result will be 1.
template <std::unsigned_integral T>
...
[[nodiscard]] constexpr uint32_t RoundUpToNextPow2Portable(uint32_t x) noexcept
{
...
[[nodiscard]] constexpr uint64_t RoundUpToNextPow2Portable(uint64_t x) noexcept
{
...
[[nodiscard]] constexpr uint32_t RoundUpToPow2Portable(uint32_t x) noexcept
{
    assert(x <= 0x80000000u);
    if (x > 1)
    {
...
[[nodiscard]] constexpr uint64_t RoundUpToPow2Portable(uint64_t x) noexcept
{
    assert(x <= 0x8000000000000000u);
    if (x > 1)
    {
