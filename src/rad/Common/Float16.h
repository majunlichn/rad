...
// https://github.com/pytorch/pytorch/blob/main/torch/headeronly/util/Half.h
...
inline uint32_t fp16_ieee_to_fp32_bits(uint16_t h)
{
    ...
    unsigned long nonsign_bsr = 0;
    uint32_t renorm_shift =
        _BitScanReverse(&nonsign_bsr, (unsigned long)nonsign) ? (uint32_t)nonsign_bsr ^ 31 : 0;
#else
    uint32_t renorm_shift = nonsign ? __builtin_clz(nonsign) : 0;
#endif
...
struct alignas(2) Float16Portable
{
    ...
    explicit Float16Portable(float value) :
        m_bits(fp16_ieee_from_fp32_value(value))
    {
    }

    static constexpr Float16Portable FromBits(uint16_t bits) noexcept
    {
        Float16Portable f;
        f.m_bits = bits;
        return f;
    ...
    operator float() const { return fp16_ieee_to_fp32_value(m_bits); }
    ...
    // We cast to float first to ensure correct IEEE behavior,
    // such as +0.0 == -0.0 and NaN != NaN, which a raw m_bits comparison would fail.
    bool operator==(const Float16Portable& rhs) const
    {
        return static_cast<float>(*this) == static_cast<float>(rhs);
    }

    auto operator<=>(const Float16Portable& rhs) const
    {
        return static_cast<float>(*this) <=> static_cast<float>(rhs);
    }

    bool is_nan() const { return (m_bits & 0x7FFF) > 0x7C00; }

    bool is_inf() const { return (m_bits & 0x7FFF) == 0x7C00; }
...
template <>
class numeric_limits<rad::Float16Portable>
{
public:
    ...
    static constexpr rad::Float16Portable min() noexcept
    {
        return rad::Float16Portable::FromBits(0x0400);
    }
    static constexpr rad::Float16Portable max() noexcept
    {
        return rad::Float16Portable::FromBits(0x7BFF);
    }
    static constexpr rad::Float16Portable lowest() noexcept
    {
        return rad::Float16Portable::FromBits(0xFBFF);
    }
    static constexpr rad::Float16Portable epsilon() noexcept
    {
        return rad::Float16Portable::FromBits(0x1400);
    }
    static constexpr rad::Float16Portable infinity() noexcept
    {
        return rad::Float16Portable::FromBits(0x7C00);
    }
    static constexpr rad::Float16Portable quiet_NaN() noexcept
    {
        return rad::Float16Portable::FromBits(0x7FFF);
    }
