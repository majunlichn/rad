...
[[nodiscard]] auto Slice(Range&& r, ptrdiff_t start, ptrdiff_t stop, ptrdiff_t step = 1)
{
    ...
    if (step > 0)
    {
        ...
        auto sliced = std::forward<Range>(r) | ranges::views::slice(start, stop) |
    ...
    else
    {
        ...
        auto sliced = std::forward<Range>(r) | ranges::views::slice(stop + 1, start + 1) |
...
[[nodiscard]] std::vector<size_t> SortIndices(Range&& r,
                                              ...
                                              Compare comp = {})
{
    ...
    std::sort(policy, indices.begin(), indices.end(),
              [&](size_t i, size_t j) { return comp(it[i], it[j]); });
    return indices;
}
...
template <std::ranges::random_access_range Range,
          typename ExecutionPolicy = std::execution::sequenced_policy,
          typename Compare = std::ranges::less>
[[nodiscard]] std::vector<size_t> StableSortIndices(Range&& r,
                                                    ExecutionPolicy policy = std::execution::seq,
                                                    Compare comp = {})
{
    const auto count = std::ranges::size(r);
