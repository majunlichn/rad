 ...
 */
[[nodiscard]] inline float fp8e4m3fn_to_fp32_value(uint8_t input)
{
    /*
    ...
    uint32_t renorm_shift = nonsign != 0 ? __builtin_clz(nonsign) : sizeof(uint32_t) * CHAR_BIT;
     ...
     */
    uint32_t result =
        sign | ((((nonsign << renorm_shift >> 4) + ((0x78 - renorm_shift) << 23)) | inf_nan_mask) &
                ~zero_mask);
 ...
 */
[[nodiscard]] inline uint8_t fp8e4m3fn_from_fp32_value(float f)
{
    /*
 ...
 * @note The implementation doesn't use any floating-point operations.
 */
[[nodiscard]] inline float fp8e5m2_to_fp32_value(uint8_t input)
{
    /*
 ...
 */
[[nodiscard]] inline uint8_t fp8e5m2_from_fp32_value(float f)
{
    /*
...
struct alignas(1) Float8E4M3
{
    uint8_t m_bits;

    Float8E4M3() = default;

    explicit Float8E4M3(float value) { m_bits = fp8e4m3fn_from_fp32_value(value); }

    ~Float8E4M3() = default;

    operator float() const { return fp8e4m3fn_to_fp32_value(m_bits); }

    Float8E4M3& operator=(float rhs)
    {
        m_bits = fp8e4m3fn_from_fp32_value(rhs);
        return *this;
    }

    Float8E4M3& operator+=(float rhs)
    {
        m_bits = fp8e4m3fn_from_fp32_value(fp8e4m3fn_to_fp32_value(m_bits) + rhs);
        return *this;
    }
    ...
    friend Float8E4M3 operator+(Float8E4M3 lhs, Float8E4M3 rhs)
    {
        return Float8E4M3(static_cast<float>(lhs) + static_cast<float>(rhs));
    }
...
struct alignas(1) Float8E5M2
{
    uint8_t m_bits;

    Float8E5M2() = default;

    explicit Float8E5M2(float value) { m_bits = fp8e5m2_from_fp32_value(value); }

    ~Float8E5M2() = default;

    operator float() const { return fp8e5m2_to_fp32_value(m_bits); }

    Float8E5M2& operator=(float rhs)
    {
        m_bits = fp8e5m2_from_fp32_value(rhs);
        return *this;
    }

    Float8E5M2& operator+=(float rhs)
    {
        m_bits = fp8e5m2_from_fp32_value(fp8e5m2_to_fp32_value(m_bits) + rhs);
        return *this;
    }
    ...
    friend Float8E5M2 operator+(Float8E5M2 lhs, Float8E5M2 rhs)
    {
        return Float8E5M2(static_cast<float>(lhs) + static_cast<float>(rhs));
    }
