...
bool Application::Init(int& argc, char**& argv)
{
    ...
    if (argc > 1)
    {
        SPDLOG_INFO("Command Line Arguments: {}", args);
    }
    SPDLOG_INFO("Working Directory: {}", GetWorkingDirectory());

    SPDLOG_INFO("User: {}", getlogin());
    SPDLOG_INFO("PID: {}", getpid());

#if defined(CPU_FEATURES_ARCH_X86)
    const auto& cpuInfo = rad::GetX86Info();
    SPDLOG_INFO("CPU: {} ({})", rad::StrTrim(cpuInfo.brand_string), rad::StrTrim(cpuInfo.vendor));
#endif
...
std::string Application::GetExecutablePath() const
{
    ...
    if (length > 0)
    {
        return StrFromWide(std::wstring(buffer.data(), length));
    }
    else
    {
        return {};
