...
{
public:
    VulkanImage(Ref<VulkanDevice> device, const vk::ImageCreateInfo& imageInfo,
    ...
    Ref<VulkanImageView> CreateView(vk::ImageViewType type, vk::Format format,
                                    const vk::ImageSubresourceRange& range,
                                    vk::ComponentMapping components = vk::ComponentMapping());
    Ref<VulkanImageView> CreateView(vk::ImageViewType type, vk::Format format);
    Ref<VulkanImageView> CreateView(vk::ImageViewType type);
    Ref<VulkanImageView> CreateView2D(uint32_t baseMipLevel = 0, uint32_t levelCount = 1,
...
public:
    ...
    VulkanDevice* GetDevice() const { return m_image->GetDevice(); }
    const vk::detail::DispatchLoaderDynamic& GetDispatcher() const;
    VulkanImage* GetImage() const { return m_image.get(); }
    vk::ImageView GetHandle() const { return m_handle; }
